...
def create_backup():
    ...
    # Use sqlite3.Connection.backup() for a safe online backup instead of
    # shutil.copy2(), which can produce a corrupt file if a write transaction
    # is in progress when the copy runs.
...
def start_scheduler():
    # TODO(audit-2026-08-30): start_scheduler() blocks in an infinite loop and
    # must be called from a daemon thread, e.g. from app/main.py lifespan:
    #   import threading
    #   from db.backup import start_scheduler
    #   threading.Thread(target=start_scheduler, daemon=True).start()
    # Not wired up yet — the backup scheduler is currently inactive.
