...
def app_client(temp_db, monkeypatch):
    ...
    The client is used as a context manager so FastAPI's lifespan runs,
    which starts the background worker thread.  Without the context manager
    the lifespan (and therefore start_worker()) is never called and every
    scan stays stuck in 'pending'.
    """
    import main
